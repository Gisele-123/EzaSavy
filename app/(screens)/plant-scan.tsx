...
export default function PlantScanScreen() {
  ...
  const handleScan = async (imageUri: string) => {
    ...
    try {
      ...
      const response = "Based on the image analysis, this appears to be a healthy tomato plant. The leaves show good color and structure, with no visible signs of disease. The plant is in its flowering stage, which is optimal for fruit production.";
      ...
      if (responseType === 'voice') {
        ...
        Alert.alert(
          "Analysis Complete",
          "Plant analysis has been completed successfully!",
          [{ text: "OK" }]
        );
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    if (!image && !plantDescription && !audioUri) {
      Alert.alert(
        "No Input",
    ...
    try {
      ...
      if (image) {
        response += "• Image Analysis: The plant appears to be healthy with good leaf color and structure.\n";
      }
      if (plantDescription) {
        response += `• Text Description: ${plantDescription}\n`;
      }
      if (audioUri) {
        response += "• Voice Description: [Audio recording analyzed]\n";
      }
      
      response += "\nOverall Analysis: The plant is showing optimal growth with no visible signs of disease. It's in its flowering stage, which is ideal for fruit production. Regular watering and sunlight exposure are recommended.";
      ...
      <ScrollView style={styles.scrollView}>
        ...
        >
          <BlurView intensity={20} style={styles.card}>
            <View style={styles.header}>
              <MaterialIcons name="local-florist" size={32} color="#4CAF50" />
              <Text style={styles.title}>Plant Analysis</Text>
            </View>

            <Text style={styles.description}>
              Analyze your plant to get detailed information about its health, species, and care recommendations.
            </Text>
              ...
              <View style={styles.imageContainer}>
                ...
                {scanning && (
                  <View style={styles.scanningOverlay}>
                    <MaterialIcons name="local-florist" size={40} color="#FFFFFF" />
            ...
            <View style={styles.textInputContainer}>
              <Text style={styles.inputLabel}>Describe the plant's appearance:</Text>
              <TextInput
                style={styles.textInput}
                multiline
                numberOfLines={4}
                placeholder="Enter plant color, size, and other visual characteristics..."
            ...
            <TouchableOpacity 
              style={[styles.button, styles.submitButton, (!image && !plantDescription && !audioUri) && styles.submitButtonDisabled]}
              onPress={handleSubmit}
              disabled={!image && !plantDescription && !audioUri || scanning}
            >
        ...
        >
          <BlurView intensity={20} style={styles.instructionsContent}>
            <Text style={styles.instructionsTitle}>How to Scan Plant</Text>
            <Text style={styles.instructionsText}>
              1. Place the plant in good lighting{'\n'}
              2. Hold the camera steady{'\n'}
              3. Ensure the entire plant is in frame{'\n'}
              4. Take a clear photo{'\n'}
              5. Describe the plant's appearance{'\n'}
